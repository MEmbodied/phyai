...
<Steps>
  ...
  <Step title="Prepare a request and construct the engine">
    ...
    With CUDA graphs enabled, `capture_profiles` are stabilized and captured while the engine is constructed. Profile outputs are discarded, and the first matching request uses graph replay directly. Pass a prepared request representative of the inputs this engine will serve; the fixed LIBERO example needs only the profile shown above. Add profiles only if the same engine will serve other sequence-length buckets, camera layouts, batch shapes, or embodiment combinations. An unlisted structure falls back to safe lazy preparation.
