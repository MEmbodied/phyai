...
<Steps>
  ...
  <Step title="准备请求并构造 engine">
    ...
    启用 CUDA Graph 后，`capture_profiles` 会在 engine 构造期间完成 workspace 稳定和图捕获，profile 的输出会被丢弃；第一个匹配的正式请求可直接回放图。传入一份能代表当前 engine 实际输入的 prepared request 即可；固定配置的 LIBERO 示例只需要上面这一份 profile。只有同一个 engine 还会处理其他序列长度 bucket、相机布局、batch shape 或具身组合时，才需要增加 profile。未列出的输入结构仍会走安全的按需准备路径。
