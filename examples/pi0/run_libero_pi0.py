...
def rollout_one(
    *,
    ...
    if save_video and frames and video_path is not None:
        ...
        write_video(
            str(video_path), np.stack(frames), env.unwrapped.metadata["render_fps"]
        )
        video_paths.append(str(video_path))

    return {
        "sum_rewards": reward_t.sum(dim=1).tolist(),
        "max_rewards": reward_t.max(dim=1).values.tolist()
        if reward_t.numel()
        else [0.0],
...
def main() -> None:
    ...
    parser.add_argument(
        "--assets-root",
        type=Path,
        default=Path(os.environ["LIBERO_ASSETS"])
        if "LIBERO_ASSETS" in os.environ
        else None,
    ...
    try:
            ...
            for task_id, env in task_map.items():
                ...
                for episode_idx in range(args.n_episodes):
                    video_path = (
                        output_dir
                        / "videos"
                        / f"{task_group}_{task_id}"
                        / f"eval_episode_{episode_idx}.mp4"
                        if save_video
                        else None
                    )
                    result = rollout_one(
        ...
        overall = {
            "avg_sum_reward": float(np.mean(all_sum_rewards))
            if all_sum_rewards
            else 0.0,
